...
static TokenType identifierType(Scanner *scanner) {
    switch (scanner->start[0]) {
        ...
        case 'c':
            ...
            break;
        case 'd':
            return checkKeyword(scanner, 1, 5, "ouble", TOKEN_DOUBLE);
        case 'e':
            ...
            if (scanner->current - scanner->start > 1) {
                switch (scanner->start[1]) {
                    case 'f':
                        return TOKEN_IF;
                    case 'n':
                        return checkKeyword(scanner, 2, 5, "teger", TOKEN_INTEGER);
                }
            }
            break;
        case 'n':
...
static Token number(Scanner *scanner) {
    ...
    if (peek(scanner) == '.' && isDigit(peekNext(scanner))) {
        ...
        while (isDigit(peek(scanner))) advance(scanner);
        return makeToken(scanner, TOKEN_REAL);
    }

    return makeToken(scanner, TOKEN_INT);
}

static Token decimal(Scanner *scanner) {
    while (isDigit(peek(scanner))) advance(scanner);
    return makeToken(scanner, TOKEN_REAL);
}
...
Token scanToken(Scanner *scanner) {
    ...
    switch (c) {
        ...
        case '.': {
            if (isDigit(peek(scanner))) return decimal(scanner);
        ...
        case '%':
            return makeToken(scanner, TOKEN_MOD);
        case '|': {
            if (match(scanner, '|')) {
