...
EXPECT_EQ(fmt::format(fmt::wformat_string<int>(L"{}"), 42),
L"42");
#if defined(FMT_USE_STRING_VIEW) && FMT_CPLUSPLUS >= 201703L
EXPECT_EQ(fmt::format(FMT_STRING(std::wstring_view(L"{}")), 42),
L"42");
#endif
}
...
namespace std {
    ...
    template<>
    struct char_traits<custom_char> {
...
EXPECT_EQ(fmt::format(L"{}", test::struct_as_wstring_view()),
L"foo");
}
...
EXPECT_EQ(fmt::format(L"{:%F}", tm),
L"2016-04-25");
EXPECT_EQ(fmt::format(L"{:%T}", tm),
L"11:22:33");
}
...
auto m = std::map<std::wstring, int>{{L"one",   1},
                                     {L"t\"wo", 2}};
EXPECT_EQ(fmt::format(L"{}", m),
L"{\"one\": 1, \"t\\\"wo\": 2}");
}
...
L"[\"\\n\\r\\t\\\"\\\\\"]");
EXPECT_EQ(fmt::format(L"{}", vec{L"понедельник"}),
L"[\"понедельник\"]");
}
...
EXPECT_EQ(fmt::format(loc, "{:L}", 1.23),
"1?23");
EXPECT_EQ(fmt::format(loc, "{:Lf}", 1.23),
"1?230000");
EXPECT_EQ(fmt::format(loc, "{:L}", 1234.5),
"1~234?5");
EXPECT_EQ(fmt::format(loc, "{:L}", 12000.0),
"12~000");
EXPECT_EQ(fmt::format(loc, "{:8L}", 1230.0),
"   1~230");
EXPECT_EQ(fmt::format(loc, "{:15.6Lf}", 0.1),
"       0?100000");
EXPECT_EQ(fmt::format(loc, "{:15.6Lf}", 1.0),
"       1?000000");
EXPECT_EQ(fmt::format(loc, "{:15.6Lf}", 1e3),
"   1~000?000000");
}
...
EXPECT_EQ(fmt::format(std::locale(), L"{:L}", -50),
L"-50");
}
...
EXPECT_EQ(fmt::format(L"{}", std::optional{L'C'}),
L"optional(\'C\')");
