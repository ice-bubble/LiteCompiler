...
EXPECT_EQ(fmt::format("{}", path("/usr/bin")),
"/usr/bin");
EXPECT_EQ(fmt::format("{:?}", path("/usr/bin")),
"\"/usr/bin\"");
EXPECT_EQ(fmt::format("{:8}", path("foo")),
"foo     ");

EXPECT_EQ(fmt::format("{}", path("foo\"bar")),
"foo\"bar");
EXPECT_EQ(fmt::format("{:?}", path("foo\"bar")),
"\"foo\\\"bar\"");

EXPECT_EQ(fmt::format("{:g}", path("/usr/bin")),
"/usr/bin");
#  ifdef _WIN32
EXPECT_EQ(fmt::format("{}", path("C:\\foo")),
"C:\\foo");
EXPECT_EQ(fmt::format("{:g}", path("C:\\foo")),
"C:/foo");
...
EXPECT_EQ(fmt::format("{}", o),
"optional(\"path/file.txt\")");
EXPECT_EQ(fmt::format("{:?}", o),
"optional(\"path/file.txt\")");
#  endif
}
...
EXPECT_FALSE(fmt::format("{}", std::this_thread::get_id())
.
...
#ifdef __cpp_lib_optional
EXPECT_EQ(fmt::format("{}", std::optional<int>{}),
"none");
EXPECT_EQ(fmt::format("{}", std::pair{1, "second"}),
"(1, \"second\")");
...
"optional(\"string\")");
EXPECT_EQ(fmt::format("{}", std::optional{'C'}),
"optional(\'C\')");
EXPECT_EQ(fmt::format("{:.{}f}", std::optional{3.14}, 1),
"optional(3.1)");
...
#ifdef __cpp_lib_optional
EXPECT_EQ(fmt::format("{}", std::optional<my_nso::my_number>{}),
"none");
EXPECT_EQ(fmt::format("{}", std::optional{my_nso::my_number::one}),
"optional(\"first\")");
EXPECT_EQ(fmt::format("{}", std::optional<my_nso::my_class>{}),
"none");
...
namespace fmt {
    template<>
    struct formatter<throws_on_move> : formatter<string_view> {
...
V0 v2("hello");
V0 v3('i');
EXPECT_EQ(fmt::format("{}", v0),
"variant(42)");
EXPECT_EQ(fmt::format("{}", v1),
"variant(1.5)");
EXPECT_EQ(fmt::format("{}", v2),
"variant(\"hello\")");
EXPECT_EQ(fmt::format("{}", v3),
"variant('i')");
...
"variant(monostate)");
EXPECT_EQ(fmt::format("{}", v5),
"variant(\"yes, this is variant\")");

volatile int i = 42;  // Test compile error before GCC 11 described in #3068.
EXPECT_EQ(fmt::format("{}", i),
"42");
...
v6.
emplace<throws_on_move>(std::move(thrower)
);
} catch (const std::runtime_error&) {
}
// v6 is now valueless by exception

EXPECT_EQ(fmt::format("{}", v6),
...
std::bitset<2> bs(1);
EXPECT_EQ(fmt::format("{} {}", bs[0], bs[1]),
"true false");
std::vector<bool> v = {true, false};
EXPECT_EQ(fmt::format("{} {}", v[0], v[1]),
"true false");
}
...
EXPECT_EQ(fmt::format("{}", bs),
"101010");
EXPECT_EQ(fmt::format("{:0>8}", bs),
"00101010");
EXPECT_EQ(fmt::format("{:-^12}", bs),
"---101010---");
}
...
EXPECT_EQ(fmt::format("{}", b),
"false");

const std::atomic<bool> cb(true);
EXPECT_EQ(fmt::format("{}", cb),
"true");
}
