...
"[1, 2, 3, 5, 7, 11]");
EXPECT_EQ(fmt::format("{::#x}", v),
"[0x1, 0x2, 0x3, 0x5, 0x7, 0xb]");
EXPECT_EQ(fmt::format("{:n:#x}", v),
"0x1, 0x2, 0x3, 0x5, 0x7, 0xb");

auto vc = std::vector<char>{'a', 'b', 'c'};
auto vvc = std::vector<std::vector<char>>{vc, vc};
EXPECT_EQ(fmt::format("{}", vc),
"['a', 'b', 'c']");
EXPECT_EQ(fmt::format("{}", vvc),
"[['a', 'b', 'c'], ['a', 'b', 'c']]");
EXPECT_EQ(fmt::format("{:n}", vvc),
"['a', 'b', 'c'], ['a', 'b', 'c']");
EXPECT_EQ(fmt::format("{:n:n}", vvc),
"'a', 'b', 'c', 'a', 'b', 'c'");
EXPECT_EQ(fmt::format("{:n:n:}", vvc),
"a, b, c, a, b, c");
}
...
FMT_BEGIN_NAMESPACE
        template<>
        struct formatter<not_default_formattable> {
...
"[\"\\n\\r\\t\\\"\\\\\"]");
EXPECT_EQ(fmt::format("{}", vec{"\x07"}),
"[\"\\x07\"]");
EXPECT_EQ(fmt::format("{}", vec{"\x7f"}),
"[\"\\x7f\"]");
EXPECT_EQ(fmt::format("{}", vec{"n\xcc\x83"}),
"[\"n\xcc\x83\"]");
...
EXPECT_EQ(fmt::format("{}", vec{"\xf0\xaa\x9b\x9e"}),
"[\"\\U0002a6de\"]");
// Broken utf-8.
EXPECT_EQ(fmt::format("{}", vec{"\xf4\x8f\xbf\xc0"}),
"[\"\\xf4\\x8f\\xbf\\xc0\"]");
EXPECT_EQ(fmt::format("{}", vec{"\xf0\x28"}),
"[\"\\xf0(\"]");
EXPECT_EQ(fmt::format("{}", vec{"\xe1\x28"}),
"[\"\\xe1(\"]");
...
"[['x']]");
EXPECT_EQ(fmt::format("{}", std::tuple<std::vector<char>>{{'x'}}),
"(['x'])");
}
...
EXPECT_EQ(fmt::format("{}", std::vector<char>{'a', 'b'}),
"['a', 'b']");
}
...
"[1, 2]");
EXPECT_EQ(fmt::format("{}", const_cast<const decltype(s) &>(s)),
"[1, 2]");
}
