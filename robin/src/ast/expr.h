...
public:
    ...
    Unary(size_t line, token::Token op, SharedPtr<Expr> right)
            : line(line), op(std::move(op)), right(std::move(right)) {}
};

/**
 * @brief 表示表达式中的自增自减一元操作。
 */
class IncrUnary : Expr {
public:
    size_t line;                   ///< 表达式所在行号
    SharedPtr<Expr> left;          ///< 要递增的表达式
    token::Token op;               ///< 操作符token

    IncrUnary(size_t line, SharedPtr<Expr> expression, token::Token op)
            : line(line), left(std::move(expression)), op(std::move(op)) {}
};
